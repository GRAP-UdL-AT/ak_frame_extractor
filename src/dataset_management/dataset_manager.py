...
class DatasetManager:
    ...
    def create_labeled_XML_files(self):
        ...
        for annotated_csv_file_name in annotated_csv_file_list:
            if annotated_csv_file_name.endswith(".csv"):
                annotated_csv_file_path = os.path.join(self.dataset_config.dataset_annotations_path, annotated_csv_file_name)
                annotated_base_name = annotated_csv_file_name.split(self.dataset_config.csv_extension)[0]
                # get image data info
                image_file_name = annotated_base_name + self.dataset_config.img_extension
                image_path = os.path.join(self.dataset_config.dataset_images_path, image_file_name)
                (width, height) = PIL.Image.open(image_path).size
                annotations_df = pd.read_csv(annotated_csv_file_path, header=None)
                # here we create an xml header
                f = ET.Element("annotations")
                ET.SubElement(f, 'filename').text = image_file_name
                ...
                filename = os.path.join(self.dataset_config.dataset_squares_path, annotated_base_name + ".xml")
    ...
    def get_labeled_list_files(self):
        """
        Returns a list structure used in while cycles to process files
        ...
        for a_filename in os.listdir(self.dataset_config.dataset_images_path):
            if a_filename.endswith(self.dataset_config.img_extension):
                ...
                file_search_depth = os.path.join(self.dataset_config.dataset_images_path, selected_filename[:-1] + self.dataset_config.suffix_depth + self.dataset_config.depth_extension)
                file_search_ir = os.path.join(self.dataset_config.dataset_images_path, selected_filename[:-1] + self.dataset_config.suffix_IR + self.dataset_config.depth_extension)
    ...
    def get_labeled_mask_list_files(self):
        """
                Returns a list structure used in loop to process files. This is a copy of the method above, but works
                with masks files
        ...
        for a_filename in os.listdir(self.dataset_config.dataset_images_path):
            if a_filename.endswith(self.dataset_config.img_extension):
                ...
                file_search_ir = os.path.join(self.dataset_config.dataset_images_path, selected_filename[:-1] + self.dataset_config.suffix_IR + self.dataset_config.depth_extension)
                # get file correspondence in another folder
                file_search_labels = os.path.join(self.dataset_config.dataset_squares_path, selected_filename + self.dataset_config.xml_extension)
